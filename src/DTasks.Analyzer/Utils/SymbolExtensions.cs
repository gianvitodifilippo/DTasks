...
internal static class SymbolExtensions
{
    ...
    private const string DTasksQualifiedName = "DTasks.DTask";
    private const string DAsyncAttributeQualifiedName = "DTasks.Metadata.DAsyncAttribute";
    ...
    {
        return type.Arity == 0 && type.QualifiedNameIs(DTasksQualifiedName.AsSpan());
    }
    
    public static bool IsGenericDTask(this INamedTypeSymbol type, [NotNullWhen(true)] out ITypeSymbol? typeArgument)
    {
        if (type.Arity != 1 || !type.QualifiedNameIs(DTasksQualifiedName.AsSpan()))
        {
            typeArgument = null;
            return false;
        }
    ...
    public static bool QualifiedNameIs(this INamedTypeSymbol type, ReadOnlySpan<char> fullName)
    {
    ...
    public static bool IsDAsync(this IMethodSymbol method)
    {
        if (!method.IsAsync)
            return false;

        ImmutableArray<AttributeData> returnTypeAttributes = method.ReturnType.GetAttributes();
        foreach (AttributeData returnTypeAttribute in returnTypeAttributes)
        {
            if (returnTypeAttribute.AttributeClass is not { } attributeClass)
                continue;

            if (attributeClass.QualifiedNameIs(DAsyncAttributeQualifiedName.AsSpan()))
                return true;
        }
